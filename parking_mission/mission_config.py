...
class Route:
    ...
    # 이 경로를 포기할 때 **지나온 자취를 되짚어 후진할 최대 거리** (m).
    # 0 = 제한 없음, 즉 이 경로를 시작한 지점까지 전부 되짚어 돌아간다.
    #
    # 왜 되짚나: 좁은 통로 안에서 막혀 멈춘 상태라면 거기서 다른 통로 입구로
    # 가는 경로를 Nav2가 못 만든다. 차를 돌릴 공간이 없기 때문이다. 먼저
    # 빠져나와야 한다.
    #
    # 왜 '자취를 되짚나': 예전에는 그냥 곧게 N미터 후진했다. 통로가 곧을 때만
    # 맞는 얘기다. A->B처럼 굽은 길을 조금 가다가 막히면 그 직선 후진이 벽을
    # 향한다. 실제로 지나온 길을 그대로 되짚으면 항상 안전하다 - 이미 그 길로
    # 왔으니 그 길이 비어 있는 것이 보장된다.
    # (mission_manager가 10cm마다 자취를 남기고 geometry.trail_to_prims가
    #  그것을 원호/직선 구간으로 되돌린다. 끝점 오차 3mm 이내로 검증했다)
    #
    # 후진은 후방 초음파 감시를 그대로 받고, 되짚기 경로도 지도 충돌 검사를
    # 통과해야 실행된다.
...
# B구역 '남측' 진입점. 남쪽에서 올라오는 경로 전용이다.
#
# 왜 따로 두나: B_STAGING(2.80, 2.28)은 남향(-90도)으로 서야 하는 자리다.
# 북쪽에서 내려오면 자연스럽지만, 남측 통로로 올라오면서 이 자리에 서려면
# 진입점을 지나쳐 북쪽 B_APPROACH(2.80, 2.73)까지 갔다가 180도 돌아 내려와야
# 한다. 폭 좁은 통로 안에서의 U턴이고, 동측 A진입에서 겪은 것과 같은 실패다.
#
# 대신 슬롯보다 **남쪽**에 남향으로 선다. 그러면 거기서부터 후진(=북상)만으로
# 슬롯에 들어간다. 진입 방향과 주차 방향이 한 줄로 이어져 선회가 90도 한 번뿐이다.
#
# y=0.95인 이유: 방위 -90도로 세워두고 실측 지도에 기동을 스윕해봤다.
#   y=0.80  여유 0.221m   y=0.95  여유 0.221m   y=1.10  여유 0.221m
#   y=0.60  여유 0.050m (차 앞코가 남쪽 벽에 붙는다)   y=0.40  해 없음
# 0.95는 안전 구간 한가운데다. 이 자리 자체의 차체 여유도 0.40m로 넉넉하다.
#
# 주의: 방위가 중요하다. 같은 자리라도 -90도가 아니면 해가 없다
#   (-90도 가능 / -70,-45,-20,0도 전부 불가). 그래서 바로 앞 경유점을
#   (2.30, 1.20)에 두어 남향으로 꺾어 들어오게 했다.
B_SOUTH_STAGING = Pose2D(2.85, 0.95, -math.pi / 2.0)
...
LEG_A_TO_B = Leg('B구역 진입', [
    ...
    Route('북측통로-남측', _wp([(0.85, 4.30), (0.97, 4.00), (0.95, 3.10),
                            (0.95, 2.20), (0.95, 1.45),
                            (1.60, 1.20), (2.30, 1.20)], B_SOUTH_STAGING),
          max_attempts=2, margin=0.15,
          note='북측통로로 남하 후 남측을 동향, 슬롯 남쪽에 남향으로 서서 '
               '후진으로 올라가 주차한다. 북측 가장자리가 막혔을 때. '
               '예전에는 (2.80,1.30)에서 북쪽 B_APPROACH(2.80,2.73)까지 올라갔다가 '
               '180도 돌아 내려왔다 - 좁은 통로 U턴이라 매번 헤맸다. '
               '동향 구간을 y=1.45가 아니라 y=1.20으로 내린 이유는 여유가 '
               '0.15에서 0.40m로 늘기 때문이다'),
])
